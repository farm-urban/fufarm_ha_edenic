...
class _FakeResponse:
    def __init__(self, status_code: int, payload) -> None:
        """Initialize a fake API response."""
...
def test_get_devices_auth_error():
    ...
    with (
        patch("requests.get", return_value=_FakeResponse(401, {})),
        pytest.raises(EdenicAuthError),
    ):
...
def test_get_devices_other_error():
    ...
    with (
        patch("requests.get", return_value=_FakeResponse(500, {})),
        pytest.raises(EdenicApiError),
    ):
        get_devices("org", "key")
